...
import { isEqualArrays, arrayTreeFilter } from '../util';
...
class Cascader extends React.Component<CascaderProps, CascaderState> {
  ...
  getCurrentLevelOptions(): CascaderOption[] {
    ...
    const result = arrayTreeFilter(options, (o, level) => o[this.getFieldName('value')] === activeValue[level], {
  ...
  handleMenuSelect = (targetOption: CascaderOption, menuIndex: number, e: React.KeyboardEvent<HTMLElement>) => {
    ...
    let activeValue = this.computedActiveValue();
    if (!activeValue) {
      return;
    }
  ...
  computedActiveValue = () => {
    if (this.state.activeValue && this.state.activeValue.length === 0) {
      return ['all'];
    }
    ...
    if (options && options.length > 0) {
      menus = (
        <Menus
          ...
          value={this.computedActiveValue()}
